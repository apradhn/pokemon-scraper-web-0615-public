...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
    ...
    it "knows pokemon with the id 143 is Snorlax" do
      expect(@db.execute("SELECT name FROM pokemon WHERE name = 'Snorlax'").flatten.first).to eq("Snorlax")
    end
  ...
  describe "saving more pokemon to the database" do
    ...
    it "knows the pokemon count increases" do
      expect(@db.execute("SELECT id FROM pokemon ORDER BY id DESC").flatten.first).to eq(152)
    end

    it "has Togepi as the last pokemon" do
      expect(@db.execute("SELECT name FROM pokemon WHERE id = (SELECT id FROM pokemon ORDER BY id DESC)").flatten.first).to eq("Togepi")
    end
  end
